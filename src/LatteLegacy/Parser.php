...
namespace Daku\Nette\FormBlueprints\LatteLegacy;
...
class Parser
{
	...
	public function parse(string $input): array
	{
		...
		if (!preg_match('##u', $input)) {
			...
			throw new \Exception('Template is not valid UTF-8 stream.');

		} elseif (preg_match('#[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]#', $input, $m, PREG_OFFSET_CAPTURE)) {
			$this->line += substr_count($input, "\n", 0, $m[0][1]);
			throw new \Exception('Template contains control character \x' . dechex(ord($m[0][0])));
		}
	...
	private function contextMacro(): bool
	{
		...
		if (!empty($matches['macro'])) {
			...
			throw new \Exception('Malformed tag contents.');
		}
	}
	...
	private function match(string $re): array
	{
		if (!preg_match($re, $this->input, $matches, PREG_OFFSET_CAPTURE, $this->offset)) {
			if (preg_last_error()) {
				throw new \Exception("Regex error", preg_last_error());
			}
