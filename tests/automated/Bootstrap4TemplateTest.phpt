...
class Bootstrap4TemplateTest extends TestCase
{
	...
	{
		return new Bootstrap4Template($options + ['customForms' => true, 'inputDescriptions' => true, 'inputErrros' => true]);
	}
	...
	public function testCreateButtons()
	{
		...
		Assert::same('<div class="form-group">{*select:foo*}<input n:name="foo" value="foo" class="btn btn-primary mr-2">{*/select*} {*select:bar*}<input n:name="bar" value="bar" class="btn btn-secondary">{*/select*}</div>', $actual);
	}
